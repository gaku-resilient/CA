...
"Accounting as per IFRS (International Financial Reporting Standards)",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-12">
            {Object.entries(services).map(([key, service]) => {
              ...
              const IconComponent = service.icon;
              return (
                <div key={key} className={`transition-all duration-500 ${isVisible['detailed-services'] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                  <div className="text-center space-y-6 mb-12">
                  ...
                  <div className="space-y-6">
                    {service.subcategories ? (
                      // Handle taxation services with subcategories
                      Object.entries(service.subcategories).map(([subcat, subServices], subcatIndex) => (
                        <div key={subcat} className="bg-gray-50 rounded-3xl p-8">
                          <div 
                          ...
                          <div className={`grid gap-4 transition-all duration-500 ${
                            expandedServices[`${key}-${subcatIndex}`] ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'
                          }`}>
                            {subServices.map((subService, serviceIndex) => (
                              <div key={serviceIndex} className="bg-white p-4 rounded-xl border-2 border-gray-200 hover:border-black transition-all duration-300 transform hover:scale-102 flex items-center space-x-3">
                      ...
                      <div className="grid sm:grid-cols-2 gap-6">
                        {service.services.map((serviceName, serviceIndex) => (
                          <div key={serviceIndex} className={`bg-gray-50 p-6 rounded-2xl border-2 border-gray-200 hover:border-black transition-all duration-300 transform hover:scale-105 hover:shadow-lg delay-${serviceIndex * 50}`}>
                            <div className="flex items-start space-x-4">
                              <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center flex-shrink-0">
                                <CheckCircle className="w-5 h-5 text-white" />
                              </div>
                              <div className="space-y-2">
                                <h4 className="font-black text-black text-lg">{serviceName}</h4>
                                <p className="text-gray-600 text-sm">Professional service delivered with expertise and reliability</p>
                              </div>
                            </div>
                          </div>
                        ))}
